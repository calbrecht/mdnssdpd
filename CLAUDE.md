...
- Loop prevention: packets from local IP addresses are dropped when reflect outputs exist.
...
### Transform Ordering
Transforms execute in TOML array order. A transform returning `Ok(false)` drops the packet. If no transforms are configured, original wire bytes are forwarded (zero-copy).

### TTL=0 Semantics
In mDNS, TTL=0 means "goodbye" (tells caches to flush the record). The `set_ttl` transform never overwrites TTL=0.

## File Inventory (2379 lines Rust)

| File | Lines | Purpose |
|---|---|---|
| `src/main.rs` | 144 | CLI (clap), config loading, channel setup, dispatch loop |
| `src/config.rs` | 134 | TOML config types: RouteConfig, TransformConfig, OutputConfig |
| `src/dns_util.rs` | 339 | DNS parsing + JSON types + mDNS bit masking + unit tests |
| `src/receiver.rs` | 151 | Socket creation, multicast join, blocking recv loop |
| `src/sender.rs` | 46 | Multicast send socket (IPv4 only currently) |
| `src/route.rs` | 151 | Route pipeline: dispatch, parse, filter, transform, output |
| `src/filter/mod.rs` | 545 | FilterEngine, chain loading, compiled rules, jq integration |
| `src/filter/ops.rs` | 224 | 14 filter operators + inline expression parser |
| `src/filter/path.rs` | 93 | Dot-notation path parser + JSON value resolver |
| `src/transform/mod.rs` | 200 | Transform trait, chain, RecordMatcher, section helpers |
| `src/transform/remove_records.rs` | 139 | RemoveRecords + RemoveServices + tests |
| `src/transform/set_ttl.rs` | 99 | SetTtl (preserves goodbye) + tests |
| `src/output/mod.rs` | 42 | Output trait + builder |
| `src/output/log.rs` | 45 | JSON log to stdout |
| `src/output/reflect.rs` | 27 | Multicast reflect via MdnsSender |
...
| `test-module.nix` | VM test definition (3 VMs, 2 VLANs, 10 subtests), uses the NixOS module |
